...
table = table_y.drop(labels=['TIMESTRR']).drop_duplicates()
...
table = table.merge(table_y, how="inner", left_on=['CLEAN_ID','TIMESTRR'], right_on=['LOC_ID','TIMESTRR'])

print("Grouping soil properties with basis on depth...")
for index, row in table.iterrows():
  if np.isnan(row['DEPTH']) : table.at[row,'DEPTH'] = row['DEPTH.f']
...
table['DEPTH'] = pandas.qcut(table['DEPTH.f'], q=2, labels=False)
...
for col in newtable.columns[newtable.isnull().any()]:
  ...
  for index, row in newtable.iterrows():
    if np.isnan(row[col]): newtable.at[index,col] = aux.predict(row[['LONWGS84_x','LATWGS84_x','DEPTH.f']].values.reshape(1, -1))[0]
newtable = newtable.fillna(newtable.mean())
table = newtable.drop_duplicates()

newtable = newtable.pivot_table(columns=['DEPTH'],index=['CLEAN_ID','TIMESTRR'])
newtable.columns = [ re.compile('[^a-zA-Z0-9_]').sub('',''.join(str(col))) for col in newtable.columns.values ]
table = table[['CLEAN_ID','SOILCLASS','LANDCOV','TIMESTRR']].drop_duplicates()
table = newtable.merge(table, how="inner", left_on=['CLEAN_ID','TIMESTRR'], right_on=['CLEAN_ID','TIMESTRR'])
...
for col in table.columns[table.isnull().any()]:
  ...
  for index, row in table.iterrows():
    if np.isnan(row[col]): table.at[index,col] = aux.predict(row[['LONWGS84_x0','LATWGS84_x0']].values.reshape(1, -1))[0]
...
#classifier = XGBClassifier(n_estimators=250, n_jobs=3)
#classifier = CatBoostClassifier()
classifier = sklearn.ensemble.RandomForestClassifier(n_estimators=250, n_jobs=3)
